...
def connect_database():
    if adharnoEntry.get()=='' or passwordEntry.get()=='':
        ...
        try:
            ...
            query='use userdata'
            mycursor.execute(query)
            query='create table data(adharno int auto_increment primary key not null,password varchar(20))'
            mycursor.execute(query)
        except:
            mycursor.execute('use userdata')
            query='select * from data where adharno=%s'
            mycursor.execute(query,(adharnoEntry.get()))
...
heading=Label(signup_window,text='User Name',font=('Microsoft Yahei UI Light',28,'bold'),bg='#F5F5F5')
